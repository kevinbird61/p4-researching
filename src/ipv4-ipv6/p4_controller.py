...
def writeForwardRules(p4info_helper,ingress_sw,
    dst_eth_addr,port,dst_ip_addr,v):
    """
        Install rules:
    ...
    if v is 6:
        table_entry = p4info_helper.buildTableEntry(
            table_name="Basic_ingress.ipv6_lpm",
            match_fields={
                "hdr.ipv6.dstAddr": (dst_ip_addr,128)
            },
            action_name="Basic_ingress.ipv6_forward",
            action_params={
                "dstAddr": dst_eth_addr,
                "port": port
            })
...
def writeTransRules(p4info_helper, ingress_sw, 
    dst_eth_addr, port, dst_ip_addr, srcAddr, dstAddr):
    ...
    table_entry = p4info_helper.buildTableEntry(
        ...
        action_name="Basic_ingress.ipv6_translate",
        action_parms={
            "dstAddr": dst_eth_addr,
            "ipv4srcAddr": srcAddr,
            "ipv4dstAddr": dstAddr,
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        writeForwardRules(p4info_helper,ingress_sw=s1,
            dst_eth_addr="00:00:00:00:01:01",port=1,dst_ip_addr="10.0.1.1",v=4)
        writeForwardRules(p4info_helper,ingress_sw=s1,
            dst_eth_addr="00:00:00:00:12:12",port=2,dst_ip_addr="10.0.2.2",v=4)
        # rules for ipv6 to v4 translation
        #writeTransRules(p4info_helper, ingress_sw=s1,
        #    dst_eth_addr="00:00:00:00:12:12",port=2,dst_ip_addr="fe80::200:ff:fe00:202",
        #    srcAddr="10.0.1.1",dstAddr="10.0.2.2")


        # - s2
        writeForwardRules(p4info_helper,ingress_sw=s2,
            dst_eth_addr="00:00:00:00:10:10",port=2,dst_ip_addr="10.0.1.1",v=4)
        writeForwardRules(p4info_helper,ingress_sw=s2,
            dst_eth_addr="00:00:00:00:02:02",port=1,dst_ip_addr="10.0.2.2",v=4)

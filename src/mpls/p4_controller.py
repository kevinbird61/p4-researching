...
def setDefaultDrop(p4info_helper,ingress_sw):
    ...
    table_entry_mpls = p4info_helper.buildTableEntry(
        ...
        action_name="Basic_ingress.drop",
        action_params={})
    ingress_sw.WriteTableEntry(table_entry_ipv4)
    ingress_sw.WriteTableEntry(table_entry_mpls)
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        writeMplsIngressRules(p4info_helper,ingress_sw=s1,
            dst_ip_addr="10.0.4.2",port=2,label=1000)
        writeMplsEgressRules(p4info_helper,ingress_sw=s1,
            dst_eth_addr="00:00:00:00:01:01",port=1,dst_label=1000)

        # - s2 (MPLS Core)
        writeMplsCoreRules(p4info_helper,ingress_sw=s2,
            port=2,dst_label=1000,new_label=1100)
        writeMplsCoreRules(p4info_helper,ingress_sw=s2,
            port=1,dst_label=1100,new_label=1000)

        # - s3 (MPLS Core)
        writeMplsCoreRules(p4info_helper,ingress_sw=s3,
            port=2,dst_label=1100,new_label=1200)
        writeMplsCoreRules(p4info_helper,ingress_sw=s3,
            port=1,dst_label=1200,new_label=1100)

        # - s4 (MPLS Egress)
        writeMplsEgressRules(p4info_helper,ingress_sw=s4,
            dst_eth_addr="00:00:00:00:04:02",port=1,dst_label=1200)
        writeMplsIngressRules(p4info_helper,ingress_sw=s4,
            dst_ip_addr="10.0.1.1",port=3,label=1200)

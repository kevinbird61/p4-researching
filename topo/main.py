...
# self-defined P4
from p4runtime_lib.p4_mininet import P4Host, P4Switch, P4RuntimeSwitch
...
def configureP4Switch(**switch_args):
    """ Helper class that is called by mininet to initialize
        ...
        Decide which class we can use 
        - P4Runtime Switch(gRPC), or P4Switch(thrift)
    """
    if "sw_path" in switch_args and 'grpc' in switch_args['sw_path']:
        # If found grpc in sw_path, then use P4Runtime Switch
...
class ExerciseTopo(Topo):
    ...
    def __init__(self, hosts,switches, links, log_dir, **opts):
        ...
        # link must be added in a sorted order so bmv2 port numbers are predictable
...
class ExerciseRunner:
    """
        Attributes:
            ...
            switches: dict<string,dict>     // mininet host names and their associated properties(dict)
            links   : list<dict>            // list of mininet link properties

            switch_json : string            // json of compiled p4
            bmv2_exe    : string            // name or path of the P4 switch binary (ex: simple_switch_grpc)
    ...
    def run_exercise(self):
        ...
        self.create_network()
        self.net.start()
        # some process that must happen after the net has started
        sleep(1)
        ...
        """ Starts up the mininet CLI and prints some helpful output.
            Assumes:
                - A mininet instance is stored as self.net and self.net.start() has
                  been called.
        """
        # Print out the switches/hosts status
